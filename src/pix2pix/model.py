...
class pix2pix(object):
    ...
    def generator_128_to_64(self, image, y=None):
        with tf.variable_scope("generator_128_to_64") as scope:
            ...
            print("##############e1 shape is {0}".format(e1.get_shape()))
            # e1 is (64 x 64 x self.gf_dim)
            e2 = self.g_bn_e2(conv2d(lrelu(e1), self.gf_dim*2, name='g_e2_conv'))
            print("##############e2 shape is {0}".format(e2.get_shape()))
            # e2 is (32 x 32 x self.gf_dim*2)
            e3 = self.g_bn_e3(conv2d(lrelu(e2), self.gf_dim*4, name='g_e3_conv'))
            print("##############e3 shape is {0}".format(e3.get_shape()))
            # e3 is (16 x 16 x self.gf_dim*4)
            e4 = self.g_bn_e4(conv2d(lrelu(e3), self.gf_dim*8, name='g_e4_conv'))
            print("##############e4 shape is {0}".format(e4.get_shape()))
            # e4 is (8 x 8 x self.gf_dim*8)
            e5 = self.g_bn_e5(conv2d(lrelu(e4), self.gf_dim*8, name='g_e5_conv'))
            print("##############e5 shape is {0}".format(e5.get_shape()))
            # e5 is (4 x 4 x self.gf_dim*8)
            e6 = self.g_bn_e6(conv2d(lrelu(e5), self.gf_dim*8, name='g_e6_conv'))
            print("##############e6 shape is {0}".format(e6.get_shape()))
            # e6 is (2 x 2 x self.gf_dim*8)
            e7 = self.g_bn_e7(conv2d(lrelu(e6), self.gf_dim*8, name='g_e7_conv'))
            print("##############e7 shape is {0}".format(e7.get_shape()))
