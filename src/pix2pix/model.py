...
class pix2pix(object):
    ...
    def residual_block_1_256(self, image):
        ...
        rb1 = tf.nn.relu(rb1)
        rb2 = self.g_bn_rb1_2_256(conv2d(rb1, self.output_c_dim, d_h=1, d_w=1, name='g_rb_256_conv_rb1_2'))
        rb_sum = tf.add(image, rb2, name='g_rb_256_add_rb1')
        return tf.nn.relu(rb_sum)

    def residual_block_2_256(self, image):
        rb1 = self.g_bn_rb2_1_256(conv2d(image, self.gf_dim * 2, d_h=1, d_w=1, name='g_rb_256_conv_rb2_1'))
        rb1 = tf.nn.relu(rb1)
        rb2 = self.g_bn_rb2_2_256(conv2d(rb1, self.output_c_dim, d_h=1, d_w=1, name='g_rb_256_conv_rb2_2'))
